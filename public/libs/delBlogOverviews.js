  ...
  .then((data) => {
    ...
    data.response.forEach((delblog) => {
      ...
      row.innerHTML = `
        ...
        <td style="text-align: start; vertical-align: middle; padding-left: 2rem"><img src="http://localhost:3002/${delblog.cImageUrl}" alt="Blog Overview Image" style="width: 60px; height: 60px; border-radius: 8px;"></td>
    ...
    deleteBlog.forEach((button) => {
      button.addEventListener("click", (event) => {
        ...
        fetch("http://127.0.0.1:3002/api/v2/delblogoverviews", {
          method: "POST",
          headers: {
          ...
          .then((response) => {
            if (response.ok) {
              ...
              alert("success"); // Corrected spelling
            } else {
